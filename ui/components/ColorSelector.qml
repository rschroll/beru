...
Empty {
    ...
    */
    ...
    /*
      Lists of foreground and background styles.  The foregrounds must be colors,
      the backgrounds can be colors or urls.
     */
    property var foregrounds
...
//                width: valueLabel.width + 2*selector.__contentsMargins
                color: backgrounds[selector.selectedIndex]

                Image {
                    anchors.fill: parent
                    fillMode: Image.Tile
                    source: {
                        if (backgrounds[selector.selectedIndex].slice(0, 5) == "url(.") {
                            var filename = backgrounds[selector.selectedIndex].slice(5, -1)
                            return Qt.resolvedUrl("../../html/" + filename)
                        }
                        return ""
                    }
                }
                ...
                LabelVisual {
                    ...
                    color: foregrounds[selector.selectedIndex]
        ...
        Repeater {
            ...
            Rectangle {
                ...
                Image {
                    ...
                    fillMode: Image.Tile
                    source: {
                        if (backgrounds[index].slice(0, 5) == "url(.") {
                            var filename = backgrounds[index].slice(5, -1)
                            return Qt.resolvedUrl("../../html/" + filename)
                        }
                        return ""
                    }
                ...
                Rectangle {
                    anchors.fill: parent
                    color: "transparent"
                    border.width: (index === selector.selectedIndex) ? units.dp(3) : 0
